...
export default function CatalogueClient({
  ...
  const { page, setPage, totalPages, itemsPage: articlesPage } = usePagination(filtres);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-ink/55 italic -mt-4">
        Un numéro = un article (chaque variante est distincte)
      </p>

      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative max-w-xs flex-1 min-w-[200px]">
          <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-ink/45" />
          <input
            value={recherche}
        ...
        <label className="flex items-center gap-1.5 text-xs text-ink/62 whitespace-nowrap">
          <input
            type="checkbox"
            ...
            {articlesPage.map((a) => (
              ...
              >
                ...
                <td>
                  {a.nom}
                  {!a.actif && (
                    <span className="ml-2 text-[9px] px-1.5 py-0.5 rounded-full bg-ink/10 text-ink/62">
                      Désactivé
                    </span>
                  )}
                </td>
                <td className="text-ink/62">{a.categorie}</td>
                ...
                <td className="pr-2.5 text-right space-x-1 whitespace-nowrap">
                  <button
                    ...
                    className="text-ink/55 hover:text-lime-deep p-1 rounded hover:bg-lime/10 transition-colors"
                    title="Modifier"
                  >
                    <Pencil size={13} />
                  </button>
                  <button
                    ...
                    className="text-ink/55 hover:text-ember p-1 rounded hover:bg-ember/10 transition-colors"
...
<table className="w-full text-xs">
          ...
          <tbody>
            {mouvements.length === 0 && (
              <tr><td colSpan={5} className="py-3 text-center text-ink/45 italic">Aucun mouvement récent</td></tr>
            )}
            {mouvements.map((m) => (
              <tr key={m.id} className="border-t border-ink/5">
                ...
                <td>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                    m.type === "entree" ? "bg-ok/10 text-ok" : "bg-ink/5 text-ink/62"
                  }`}>
                    {m.type === "entree" ? "Entrée" : "Sortie"}
                  </span>
                </td>
                <td className={`num ${m.type === "entree" ? "text-ok" : "text-ink/70"}`}>
                  {m.type === "entree" ? "+" : "-"}{m.quantite}
                </td>
                <td className="text-ink/62">{m.motif}</td>
              </tr>
            ))}
          </tbody>
        </table>
