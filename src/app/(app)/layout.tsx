...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="bg-lime-deep text-white px-4 sm:px-6 py-2.5 text-sm">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="leading-tight hidden sm:block">
              ...
              <div className="text-[11px] text-white/64">{profil.nom}</div>
            </div>
          </div>

          <NavLinksDesktop role={profil.role} />

          <div className="flex items-center gap-2.5 shrink-0">
            <span className="hidden sm:inline text-[10px] bg-white/15 text-white rounded-full px-2.5 py-1 uppercase tracking-wide font-semibold">
              {profil.role}
            </span>
            <DeconnexionButton />
            <NavMobileMenuButton role={profil.role} />
          </div>
        </div>
      </header>
      <SyncBar />
